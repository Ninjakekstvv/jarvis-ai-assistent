import speech_recognition as sr
from voice_state import log, set_voice_state


recognizer = sr.Recognizer()

# Sprachaufnahme optimiert für längere Sätze
recognizer.pause_threshold = 1.5
recognizer.phrase_threshold = 0.3
recognizer.non_speaking_duration = 0.8


def zuhoeren(wake_only=False):
    """Hört entweder im aktiven Modus oder unsichtbar auf das Wake-Word."""

    standby_message = "STANDBY Protokoll Aktiv schlüssenwort Jarvis"

    if wake_only:
        set_voice_state("idle", standby_message)
    else:
        set_voice_state("listening", "ICH HÖRE ZU")


    try:
        with sr.Microphone() as source:

            log("Mikrofon aktiv – ich höre zu...")


            # kurze Kalibrierung statt jedes Mal lange warten
            recognizer.adjust_for_ambient_noise(
                source,
                duration=0.5
            )


            audio = recognizer.listen(
                source,
                timeout=None,
                phrase_time_limit=20
            )


        text = recognizer.recognize_google(
            audio,
            language="de-DE"
        )


        log("Du: " + text)


        if wake_only:
            set_voice_state(
                "idle",
                standby_message
            )
        else:
            set_voice_state(
                "thinking",
                "ANALYSIERE DEINE ANFRAGE"
            )


        return text.lower()



    except sr.UnknownValueError:

        set_voice_state(
            "idle",
            standby_message if wake_only else ""
        )

        return ""



    except Exception as e:

        log(e)

        set_voice_state(
            "idle",
            standby_message if wake_only else ""
        )

        return ""



if __name__ == "__main__":
    zuhoeren()